...
namespace
{
	...
	// active state and clear input gates. Also delay the final loaded-world
	// notification if the client fires it before the pawn/PS/subsystems exist.
	// =========================================================================
	static TSet<TObjectKey<AFGPlayerController>> GDelayedLoadedWorldPending;
	static TSet<TObjectKey<AFGPlayerController>> GDelayedLoadedWorldReplay;
	static const int32 LOADED_WORLD_DELAY_MAX_ATTEMPTS = 30;
	static const float LOADED_WORLD_DELAY_INTERVAL = 0.5f;
	...
	bool AreJoinCriticalObjectsReady(AFGPlayerController* PC)
	{
		...
		if (!(PC->*HasInitialLightweightDataFn)()) return false;
		return true;
	}
	...
	void ForceActivePlayerState(AFGPlayerController* PC, const TCHAR* Reason)
	{
		...
		PC->SetDisabledInputGate(FDisabledInputGate(false));
	...
	void ReplayLoadedWorldWhenReady(TWeakObjectPtr<AFGPlayerController> WeakPC, FName WorldPackageName, bool bIsFinalDest, int32 Attempt)
	{
		AFGPlayerController* PC = WeakPC.Get();
		if (!IsValid(PC)) return;

		UWorld* World = PC->GetWorld();
		if (!IsValid(World)) return;

		const bool bReady = AreJoinCriticalObjectsReady(PC);
		if (!bReady && Attempt < LOADED_WORLD_DELAY_MAX_ATTEMPTS)
		{
			FTimerHandle Handle;
			World->GetTimerManager().SetTimer(
				Handle,
				FTimerDelegate::CreateLambda([WeakPC, WorldPackageName, bIsFinalDest, Attempt]()
				{
					ReplayLoadedWorldWhenReady(WeakPC, WorldPackageName, bIsFinalDest, Attempt + 1);
				}),
				LOADED_WORLD_DELAY_INTERVAL,
				false);
			return;
		}

		const TObjectKey<AFGPlayerController> Key(PC);
		GDelayedLoadedWorldPending.Remove(Key);
		GDelayedLoadedWorldReplay.Add(Key);

		UE_LOG(LogSinkSubsystemRaceFix, Display,
			TEXT("Hook 13: replaying NotifyLoadedWorld(%s final=%d) for %s after %.1fs ready=%d"),
			*WorldPackageName.ToString(),
			(int32)bIsFinalDest,
			*PC->GetName(),
			Attempt * LOADED_WORLD_DELAY_INTERVAL,
			(int32)bReady);

		PC->NotifyLoadedWorld(WorldPackageName, bIsFinalDest);
		ForceActivePlayerState(PC, TEXT("DelayedNotifyLoadedWorld"));
	}

	void InstallJoinCompletionHook()
	{
		SUBSCRIBE_UOBJECT_METHOD(AFGPlayerController, NotifyLoadedWorld,
			[](auto& Scope, AFGPlayerController* Self, FName WorldPackageName, bool bIsFinalDest)
			{
				if (!IsValid(Self)) return;
				UWorld* World = Self->GetWorld();
				if (!IsValid(World)) return;
				...
				const TObjectKey<AFGPlayerController> Key(Self);
				if (GDelayedLoadedWorldReplay.Contains(Key))
				{
					GDelayedLoadedWorldReplay.Remove(Key);
					return;
				}

				UE_LOG(LogSinkSubsystemRaceFix, Display,
					TEXT("Hook 13: NotifyLoadedWorld(%s final=%d) on %s ready=%d"),
					*WorldPackageName.ToString(),
					(int32)bIsFinalDest,
					*Self->GetName(),
					(int32)AreJoinCriticalObjectsReady(Self));

				if (bIsFinalDest && !AreJoinCriticalObjectsReady(Self) && !GDelayedLoadedWorldPending.Contains(Key))
				{
					GDelayedLoadedWorldPending.Add(Key);
					Scope.Cancel();
					ReplayLoadedWorldWhenReady(TWeakObjectPtr<AFGPlayerController>(Self), WorldPackageName, bIsFinalDest, 0);
				}
			});
		...
		SUBSCRIBE_UOBJECT_METHOD(AFGPlayerController, SetDisabledInputGate,
			[](auto& Scope, AFGPlayerController* Self, FDisabledInputGate NewGate)
			{
				if (!IsValid(Self)) return;
				AFGCharacterPlayer* Char = GetFGCharacterFromController(Self);
				if (!IsValid(Char) || !GetCharacterOnline(Char)) return;

				if (NewGate.mJump || NewGate.mHotbar || NewGate.mBuildGun)
				{
					UE_LOG(LogSinkSubsystemRaceFix, Display,
						TEXT("Hook 13: replacing active-player disabled-input gate on %s (Jump=%d Hotbar=%d BuildGun=%d IsRespawning=%d)"),
						*Self->GetName(),
						(int32)NewGate.mJump,
						(int32)NewGate.mHotbar,
						(int32)NewGate.mBuildGun,
						(int32)Self->IsRespawning());
					Scope.Cancel();
					Self->SetDisabledInputGate(FDisabledInputGate(false));
				}
			});

		SUBSCRIBE_UOBJECT_METHOD(AFGCharacterPlayer, CanEquipBuildGun,
			[](auto& Scope, const AFGCharacterPlayer* Self)
			{
				if (!IsValid(Self) || !GetCharacterOnline(Self)) return;
				if (Self->GetBuildGun() == nullptr) return;
				...
				if (LogBudget > 0)
				{
					LogBudget--;
					UE_LOG(LogSinkSubsystemRaceFix, Display,
						TEXT("Hook 13: allowing CanEquipBuildGun on active online character %s"),
						*Self->GetName());
				}
				Scope.Override(true);
			});

		SUBSCRIBE_UOBJECT_METHOD(AFGCharacterPlayer, CanEquipBuildGunForDismantle,
			[](auto& Scope, const AFGCharacterPlayer* Self)
			{
				if (!IsValid(Self) || !GetCharacterOnline(Self)) return;
				if (Self->GetBuildGun() == nullptr) return;
		...
		UE_LOG(LogSinkSubsystemRaceFix, Display,
			TEXT("Hook 13: join/respawn completion repair hooks installed"));
	}
	...
	bool LoadingScreenKillerTick(float /*Delta*/)
	{
		...
		// Stuck-respawn-overlay kill: identified visually from a screenshot —
		// the "RESPAWN_SEQUENCE.BAT / FICSIT TIP / INITIALIZING..." overlay
		// is `Widget_Respawn_C` from the RespawnPoints mod, added to the
		// viewport on join and never dismissed when SF's load-finished
		// signal doesn't fire. Collapse it every tick once we're in a real
		// world with a possessed pawn. Idempotent.
		if (NM != NM_Standalone && IsValid(Pawn))
		{
			static bool bLoggedRespawnKill = false;
			for (TObjectIterator<UUserWidget> It; It; ++It)
			{
				...
				if (Cls == TEXT("Widget_Respawn_C") ||
					Cls.Contains(TEXT("LoadingScreen")))
				{
					if (UW->GetVisibility() != ESlateVisibility::Collapsed)
					{
						UW->SetVisibility(ESlateVisibility::Collapsed);
						if (!bLoggedRespawnKill)
						{
							bLoggedRespawnKill = true;
							UE_LOG(LogSinkSubsystemRaceFix, Display,
								TEXT("Hook 7: Collapsed stuck respawn/loading UMG widget %s (%s)"),
								*Cls, *UW->GetName());
						}
					}
				}
